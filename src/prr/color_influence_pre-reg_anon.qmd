...
#code-repo: |
#  For replication, go to: <https://github.com/DamonCharlesRoberts/color_as_political_information>.
author:
  - name: Anonymized for peer review
...
df_select <- data[["clean"]][ # select the following columns
  , c("Female", "White", "age", "PartyId", "BlueTreatment", "RedTreatment", "Vote"), with = FALSE
] |> as.data.frame()
...
The evidence from this pre-test is limited. The experimental design is not testing pre-conscious evaluations. The treatments are explicitly political and rely upon a convenience sample of those enrolled in political science classes. The ability of individuals to associate partisanship with the treatments is likely overstated. As I discussed with the third model, I also have omitted variable bias as the result of my outcome is a better measure of approach behaviors for those who are extroverted.
...
Given that my estimates of insincere responses, of effect size, and that these simulations use models examining my moderating hypothesis, the rates of true positives are likely quite conservative. The results of my simulation suggest that I should aim for a sample size greater than 400 once I've excluded insincere responses. Combining this with funding considerations, I will recruit total sample size of 1000 participants for Study 1.
