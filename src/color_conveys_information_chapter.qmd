...
```{r}
#| label: h-2-predictions
predicted_prob <- marginaleffects::plot_predictions(
  list_fitted[["h_2"]][["red"]]
  , condition = "trial_1_red_stimuli"
) +
facet_wrap(~group) +
theme_minimal()

predicted_prob <- marginaleffects::predictions(
  list_fitted[["h_2"]][["red"]]
)|>
as.data.table()
predicted_prob_bar <- predicted_prob[
  , .(estimate = mean(estimate), conf.low = mean(conf.low), conf.high = mean(conf.high)), by = c("group", "trial_1_red_stimuli")
][
  , trial_1_red_stimuli := factor(trial_1_red_stimuli)
][
  , group := factor(group)
]

ggplot(data = predicted_prob_bar) +
geom_bar(
  aes(x = trial_1_red_stimuli, y = estimate, fill = group)
  , stat = "identity"
  , position = "dodge"
) + 
geom_errorbar(
  aes(x = trial_1_red_stimuli, ymin = conf.low, ymax = conf.high, linetype = group)
  , position = "dodge"
) +
scale_x_discrete(labels = c("Other", "Red")) +
scale_fill_manual(labels = c("Democrat", "Independent", "Republican"), values = c("#00AEF3", "#808080", "#ff0803")) +
scale_linetype_manual(labels = c("Democrat", "Independent", "Republican"), values = c("solid", "solid", "solid")) +
labs(
  x = "Color of yard sign"
  , y = "Pr(Party of candidate)"
  , linetype = "Party of candidate"
  , fill = "Party of candidate"
) +
theme_minimal()
```
